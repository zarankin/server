...
ConnectionHandler::ConnectionHandler(string host, short port, EncoderDecoder &encoderDecoder) : host_(host), port_(port), io_service_(),
                                                                socket_(io_service_), connected(
                true), encoderDecoder(encoderDecoder) {
}
...
void ConnectionHandler::decode(char nextByte) {
    encoderDecoder.DecodeNextByte(nextByte);
    if (encoderDecoder.loguot) {
        this->connected = false;
    }
